...
int udp_p2p::receive_packet()
{
    ...
    if (!connected_peer && (loop_count % 10 == 0)) {
...
void receive_setting()
{
    //printf("receive_setting()\n");
    return_receive = g_udp_p2p.receive_packet();
}
...
int main(int argc, char** argv)
{
    ...
    // menu loop
    while (true) {
        thread_receive = thread(receive_setting);

        // simulating 30fps in swos menu
        Sleep(33);

        //receive_setting();
        if (return_receive == ENET_EVENT_TYPE_DISCONNECT) {
            thread_receive.join();
            // exit menu loop
