// enet-test-starwindz-v2.6.2
/*
m.bat for building exe using msbuild
...
int udp_p2p::receive_packet()
{
    ...
    while (enet_host_service(local_host, &event, 0) > 0) {
        ...
        switch (event.type) {
            ...
            case ENET_EVENT_TYPE_DISCONNECT: {
                // only when connected_peer == event.peer, it should exit from the loop.
                // otherwise, there is a problem of being disconnected after a specific time has elapsed.
    ...
    if (!connected_peer && (loop_count % 3 == 0)) {
        connected = false;
...
GoGetTeamsForPlay
-> chooseTeamsMenu
SelectTeamsFinalMenu
   -> chooseTeamsMenu (- || -)
   -> selectTeamsMenu
   -> chooseTeamsFriendlyMenu
   -> chooseTeamsFriendly
*/
    ...
    while (true) {
        ...
        if (_kbhit()) {
            ...
            if (c == '1') {
                ...
                // since loop should exit immediately when 'exit' button clicked
                if (!g_udp_p2p.connected_peer) {
                    break;
                }
            }
        }
