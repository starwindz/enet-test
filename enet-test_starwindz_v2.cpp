...
public:
    int setup(int _local_port, const char* _remote_ip, int _remote_port, int _buffer_size);
    int init();
    int close();

    int send_buffer();
    int receive_buffer(bool send_back_ping_when_connected);
};

udp_p2p g_udp_p2p;

int udp_p2p::setup(int _local_port, const char* _remote_ip, int _remote_port, int _buffer_size)
{
...
int udp_p2p::receive_buffer(bool send_back_ping_when_connected)
{
    Sleep(1);
    loop_count++;
...
int main(int argc, char** argv)
{
    ...
    if (strcmp(argv[1], "0") == 0) {      // local-remote mode
        g_udp_p2p.setup(27885, "127.0.0.1", 27886, sizeof(MPGameSetting));
    }
    else if (strcmp(argv[1], "1") == 0) { // remote-local mode
        g_udp_p2p.setup(27886, "127.0.0.1", 27885, sizeof(MPGameSetting));
    }
    else {
        return 0;
    }
#else  
    g_udp_p2p.setup(atoi(argv[1]), argv[2], atoi(argv[3]), sizeof(MPGameSetting));
#endif
    ...
    while (g_udp_p2p.loop) {
        if (g_udp_p2p.receive_buffer(true) == ENET_EVENT_TYPE_DISCONNECT) {
            // process disconnect
            break;
        }
